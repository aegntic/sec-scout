...
class MockNucleiAdapter(ToolAdapter):
    """Mock adapter for nuclei - simulates real tool behavior"""
    
    def __init__(self):
        super().__init__("nuclei_mock", "Mock Nuclei Adapter")
        self.mock_mode = True
    
    def execute(self, target, options=None):
        """Simulate tool execution"""
        self.logger.info(f"Mock nuclei scan started on {target}")
        ...
        results = {
            ...
            "raw_output": f"Mock nuclei scan completed",
            "tool": "nuclei_mock"
        }
